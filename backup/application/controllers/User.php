...
public function blogList() {
    ...
    public function saveContent() {
        try {
            ...
            $submenuId = $this->input->post('submenu_id');
            
            if (!$menuId || !$submenuId) {
                throw new Exception('Menu and Submenu IDs are required');
            }
            ...
            $exists = $this->db->get('menu_contents')->num_rows() > 0;
            
            if ($exists) {
                $this->db->where('menu_id', $menuId);
                $this->db->where('submenu_id', $submenuId);
                $this->db->update('menu_contents', array_merge($data, ['updated_at' => date('Y-m-d H:i:s')]));
            } else {
                $this->db->insert('menu_contents', $data);
            }
            
            if ($this->db->affected_rows() > 0) {
                $response = [
                    'status' => 'success',
                    'message' => 'Content saved successfully Thank you'
                ];
            } else {
    ...
    public function getContent($menuId, $submenuId) {
        try {
            if (!$menuId || !$submenuId) {
                throw new Exception('Menu and Submenu IDs are required');
            }
            ...
            $this->db->where('menu_id', $menuId);
            $this->db->where('submenu_id', $submenuId);
            $content = $this->db->get('menu_contents')->row();
